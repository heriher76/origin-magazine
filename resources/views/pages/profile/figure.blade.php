...
<div class="col-md-12">
    ...
    <div class="tab-content">
        <div role="tabpanel" class="tab-pane active row" id="home{{$key}}">
            <div class="col-md-2">
                <img src="{{ url('figure/'.$figure->thumbnail) }}" alt="" style="max-width: 150px;">
            </div>
            <div class="col-md-10">
                <p>{!! $figure->description !!}</p>
            </div>
        </div>
        <div role="tabpanel" class="tab-pane" id="profile{{$key}}">{!! $figure->address !!}</div>
        <div role="tabpanel" class="tab-pane" id="messages{{$key}}">{!! $figure->contact !!}</div>
    </div>
    </div>
</div>
@endforeach
